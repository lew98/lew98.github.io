...
\begin{proof}
    ...
    \]
    Hence we see that \( A \sim g(A) \) via \( g \). Since \( A \) is infinite, \( g(A) \) must also be infinite. Then by \Cref{prop:infinite_subset_of_countable_set_is_countable}, \( g(A) \sim \N \). It follows that \( A \sim \N \), i.e.\ \( A \) is countably infinite.
\end{proof}
...
\begin{proof}
    By \Cref{prop:countable_iff_surjection_from_N}, it will suffice to construct a surjection \( g : \N \to \bigcup_{n=1}^{\infty} A_n \). For each \( n \in \N \) we have a bijection \( f_n : \N \to A_n \). For \( m, n \in \N \), let \( a_{mn} = f_n(m) \). As a sketch of our construction, arrange these terms in an ``infinite grid'' like so:
    \[
        \begin{tikzpicture}
...
\begin{proof}
    \begin{enumerate}[label = (\roman*)]
        \item If \( A \) is empty then the result is clear. Suppose that \( A \) is non-empty and finite, say \( A = \{ a_1, \ldots, a_k \} \) for some \( k \in \N \). By \Cref{prop:countable_iff_surjection_from_N}, there is a surjection \( f : \N \to B \). Define a function \( g : \N \to A \cup B \) by
        \[
...
\begin{proof}
    \begin{enumerate}[label = (\roman*)]
        ...
        \item Suppose \( A_j \) is countably infinite for some \( j \in I \). If \( I \) is finite, say \( \{ 1, \ldots, k \} \sim I \) via some bijection \( f \), let \( E_n = A_{f(n)} \cup A_j \) for \( 1 \leq n \leq k \) and \( E_n = A_j \) for \( n > k \). If \( I \) is countably infinite, let \( f : \N \to I \) be a bijection, and define \( E_n = A_{f(n)} \cup A_j \) for \( n \in \N \). In either case, by \Cref{cor:union_of_amc_set_and_countable_set_is_countable}, each \( E_n \) is countably infinite. Furthermore,
        \[
        ...
        \item For \( i \in J \), let \( E_i = A_i \cup \N \). Then each \( E_i \) is countably infinite by \Cref{cor:union_of_amc_set_and_countable_set_is_countable} and
        \[
...
\begin{proof}
    Let us take as our construction of \( \Q \) the set of equivalence classes of elements in \( \Z \times (\Z \setminus \{ 0 \}) \) where we declare \( (a, b) \) and \( (c, d) \) equivalent if and only if \( ad = bc \). Let \( \pi : \Z \times (\Z \setminus \{ 0 \}) \to \Q \) be the canonical map, which is always a surjection. By \Cref{prop:Z_is_countable}, \( \Z \) is countably infinite; it is straightforward to modify the proof of that proposition to see that \( \Z \setminus \{ 0 \} \) is also countably infinite. Then by \Cref{cor:finite_product_of_amc_sets_is_amc}, \( \Z \times (\Z \setminus \{ 0 \}) \) is countably infinite. Let \( f : \N \to \Z \times (\Z \setminus \{ 0 \}) \) be a bijection. Then the composition \( \pi \circ f : \N \to \Q \) is a surjection and hence by \Cref{prop:countable_iff_surjection_from_N} we see that \( \Q \) is countably infinite.
\end{proof}
...
\Cref{prop:no_surjection_from_N_to_set_of_binary_sequences} implies that the set of all binary sequences \( T \) cannot be put in bijection with \( \N \); since \( T \) is evidently infinite, it follows that \( T \) is uncountably infinite.
...
Let us now consider the cardinality of \( \R \). Using the binary representation of real numbers, one can construct a bijection between \( \R \) and the set of all binary sequences \( T \). \Cref{prop:no_surjection_from_N_to_set_of_binary_sequences} then implies that \( \R \) is uncountably infinite. However, we will take a different approach, using the \href{https://lew98.github.io/Mathematics/Miscellaneous/Nested_interval_property.pdf}{nested interval property} of \( \R \), which is much closer to Cantor's original 1874 proof of the uncountability of \( \R \).
...
Evidently, \( \R \) is infinite; \Cref{prop:no_surjection_from_N_to_R} then implies that \( \R \) is uncountably infinite. In light of \Cref{cor:union_of_amc_set_and_countable_set_is_countable}, which implies that the union of two countably infinite sets is countably infinite, and \Cref{cor:Q_is_countable}, which says that \( \Q \) is countably infinite, we see that the set of irrational numbers \( \R \setminus \Q \) must be uncountably infinite.
